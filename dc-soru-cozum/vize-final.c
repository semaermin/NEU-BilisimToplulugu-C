...
#include<string.h>
#include<locale.h>
#include <stdbool.h> // boolean de�erer kullanmak i�in gerekli k�t�phane
...
void ekle(char ad[20],int dersSay)
{
    ...
    (ogr+ogrSay)->dersler=(Ders*)malloc(sizeof(Ders)*derSay);
...
void arama(char ad[20]){
    ...
    if(bulundu == false)
    {
    	printf("Aradiginiz kisiye ulasilamadı.");
    }

}
void guncelleme(int ogrenci){
     if(ogrenci<=0||ogrenci>ogrSay){
        printf("Boyle bir ögrenci bulunmamaktadir..");
    }else{
...
int main()
{
    ...
    while(dongu)
    {
        ...
        case 1:
            ...
            scanf("%s",ad);
            printf("%s adli öğrencinin kaç dersi var?",ad);
            scanf("%d",&dersSay);
            ekle(ad,dersSay);
            gecmeNotuHesaplama();
            break;
        ...
        printf("devam etmek icin bir tusa basınız..");
        getch();
        system("CLS");
    }
